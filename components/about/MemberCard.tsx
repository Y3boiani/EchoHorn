...
const MemberCard = ({ imageUrl, name, role, description }: { imageUrl: string; name: string; role: string; description: string; }) => {
  return (
    // 1. Increased max-width for a larger card and used grid for proportional scaling
    <div
      className="grid grid-cols-1 md:grid-cols-3 items-center p-12 gap-12 rounded-3xl border border-white/10 w-full max-w-6xl bg-cover bg-center bg-[url('/rectangle-13.png')]"
    >
      ...
      <div className="text-center md:text-left md:col-span-2">
        ...
        <p className="mt-8 text-gray-200 text-xl leading-relaxed">{description}</p>
      </div>
    </div>
  );
};
